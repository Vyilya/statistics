...
class TaskForm(forms.ModelForm):
    ...
    due_time = forms.TimeField(label='Время выполнения', widget=forms.TimeInput(attrs={'type': 'time'}))  # Добавлено поле для времени
    status = forms.ChoiceField(label='Статус', choices=Task.STATUS_CHOICES)

    class Meta:
        model = Task
        fields = ['title', 'description', 'assigned_to', 'due_date', 'due_time', 'status']
